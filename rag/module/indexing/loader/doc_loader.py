    ...
    def _get_elements(self) -> List:
        def doc2text(filepath):
            ...
            b_unit = tqdm.tqdm(total=len(doc.paragraphs)+len(doc.tables),
                               desc="CustomizedOcrDocLoader block dataprep: 0")
            for i, block in enumerate(iter_block_items(doc)):
                b_unit.set_description(
                    "CustomizedOcrDocLoader block dataprep: {}".format(i))
                b_unit.refresh()
...
if __name__ == "__main__":
    # 指定 PDF 文件路径
    docx_file_path = "./data/raw/docx/example.docx"

    # 创建自定义加载器实例
    custom_loader = CustomizedOcrDocLoader(docx_file_path)
