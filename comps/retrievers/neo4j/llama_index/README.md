...
```bash
cd ../../
docker build -t opea/retriever-community-answers-neo4j:latest --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/retriever/neo4j/llama_index/Dockerfile .
```
...
```bash
cd comps/retriever/neo4j/llama_index
