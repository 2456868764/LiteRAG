    ...
    if files:
        ...
        for file in files:
            if os.path.splitext(file.filename)[1] in [".mp4", ".wav"]:
                ...
                raise HTTPException(
                    status_code=400, detail=f"File {file.filename} is not an mp4 file. Please uploaded_files mp4 files only."
                )
    ...
    if files:
        ...
        for file in files:
            if os.path.splitext(file.filename)[1] in [".mp4", ".png", ".jpg", ".jpeg", ".gif"]:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"File {file.filename} is not a supported file type. Please uploaded_files mp4, png, jpg, jpeg, and gif files only.",
                )
    ...
    if files:
        ...
        if len(matched_files.keys()) == 0:
            return HTTPException(
                status_code=400,
                detail="The uploaded files have unsupported formats. Please uploaded_files at least one video file (.mp4) with captions (.vtt) or one image (.png, .jpg, .jpeg, or .gif) with caption (.txt)",
            )
        ...
        for media_file in matched_files:
            ...
            # Save caption file in uploaded_files directory
