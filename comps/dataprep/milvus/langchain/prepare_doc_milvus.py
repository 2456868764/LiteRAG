...
async def ingest_documents(
    ...
    if logflag:
        logger.info(f"[ uploaded_files ] files:{files}")
        logger.info(f"[ uploaded_files ] link_list:{link_list}")
    ...
    if files:
        ...
        for file in files:
            ...
            if logflag:
                logger.info(f"[ uploaded_files ] processing file {save_path}")
            ...
            if my_milvus.col:
                ...
                if len(search_res) > 0:
                    if logflag:
                        logger.info(f"[ uploaded_files ] File {file.filename} already exists.")
                    raise HTTPException(
                        status_code=400,
    ...
    if link_list:
        ...
        for link in link_list:
            ...
            if logflag:
                logger.info(f"[ uploaded_files ] processing link {encoded_link}")
            ...
            if my_milvus.col:
                ...
                if len(search_res) > 0:
                    if logflag:
                        logger.info(f"[ uploaded_files ] Link {link} already exists.")
                    raise HTTPException(
        ...
        if logflag:
            logger.info(f"[ uploaded_files ] Successfully saved link list {link_list}")
...
async def delete_single_file(file_path: str = Body(..., embed=True)):
    ...
    if file_path == "all":
        ...
        if logflag:
            logger.info("[ delete ] new uploaded_files folder created.")
        return {"status": True}
